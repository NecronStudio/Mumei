...
# クリティカル率
	...
	# 増加が無かったら
	execute if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].NSD.Weapon.Mainhand.CritChance unless data storage necron: Items.tag.NSD.Status.CritChance run function necron.player:weapon/mainhand/crit.chance/remove
	# 値を取得
	...
	# 前に持っていた値より大きかったら
	execute if data storage necron: Items.tag.NSD.Status.CritChance unless score $NSD.Value NSD.Temp = $NSD.Amount NSD.Temp run function necron.player:weapon/mainhand/crit.chance/add
# クリティカルダメージ
	# 追加値を取得
	...
	# 増加が無かったら
	execute if data storage oh_my_dat: _[-4][-4][-4][-4][-4][-4][-4][-4].NSD.Weapon.Mainhand.CritDamage unless data storage necron: Items.tag.NSD.Status.CritDamage run function necron.player:weapon/mainhand/crit.damage/remove
	# 値を取得
	...
	# 前に持っていた値より大きかったら
	execute if data storage necron: Items.tag.NSD.Status.CritDamage unless score $NSD.Value NSD.Temp = $NSD.Amount NSD.Temp run function necron.player:weapon/mainhand/crit.damage/add
# 最後の処理
